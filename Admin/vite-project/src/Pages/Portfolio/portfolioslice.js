...
export const fetchTech = createAsyncThunk("portfolio/fetchTech", async (_, { rejectWithValue }) => {
  try {
    const res = await getTechApi();
    return res.data?.data || [];
  } catch (err) {
...
export const createTech = createAsyncThunk("portfolio/createTech", async (formData, { rejectWithValue }) => {
  try {
    const res = await createTechApi(formData);
    return res.data?.data;
  } catch (err) {
...
export const updateTech = createAsyncThunk("portfolio/updateTech", async ({ id, formData }, { rejectWithValue }) => {
  try {
    const res = await updateTechApi(id, formData);
    return res.data?.data;
  } catch (err) {
...
export const deleteTech = createAsyncThunk("portfolio/deleteTech", async (id, { rejectWithValue }) => {
  try {
    await deleteTechApi(id);
    return id;
  } catch (err) {
...
const initialState = {
  items: [],
  categories: [],
  form: { title: "", description: "", category: "", link: "", images: [] },
  editId: null,
  loading: false,
  tableLoading: false,
  deleteLoading: {},
...
const portfolioSlice = createSlice({
  ...
  state.form = {
    ...
    category: item.category?._id || item.category || "",
    link: item.link || "",
    images: [],           // Important: clear new images
  };
    },
    resetForm: (state) => {
      state.editId = null;
  ...
  extraReducers: (builder) => {
    builder
      .addCase(fetchTech.pending, (state) => { state.tableLoading = true; })
      .addCase(fetchTech.fulfilled, (state, action) => {
        state.tableLoading = false;
        state.items = action.payload;
      })
...
export const { setFormValue, setFormImages, beginEdit, resetForm, clearError } = portfolioSlice.actions;
export default portfolioSlice.reducer;